...
os.environ["OPENAI_API_KEY"] = "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"
...
## Instruction
- 정확히 답변을 하지 않으면 기업이 엄청난 손해를 보게된다.
- 사용자가 대화를 통해 과학 지식에 관한 주제로 질문하면 search api를 호출할 수 있어야 한다.
- 과학 상식과 관련되지 않은 나머지 대화 메시지에는 적절한 대답을 생성한다.
- 사용자가 대화를 통해 생물학, 물리학, 지구과학, 화학, 의학, 공학, 컴퓨터, 농학, 산업공학, 전자공학, 수학, 통계학, 과학자 등 과학 지식에 관한 주제로 질문하면 search api를 호출할 수 있어야 한다.
- 사회, 경제, 시사, 문화, 문학, 역사, 인문학, 사교 등 과학 지식과 관련되지 않은 나머지 대화 메시지에는 적절한 대답을 생성한다.
- 결과는 json 형태로 생성하고, 반드시 'is_science' 필드를 추가하여 질문이 과학 지식에 관련된 내용이면 true를, 과학 지식에 관련된 내용이 아니라면 false를 value로 만든다. 답변 필드의 이름은 'answer'로 통일한다.
"""
...
def answer_question(messages):
    ...
    try:
        result = client.chat.completions.create(
            model=llm_model,
            response_format={"type":"json_object"}, # 2024.10.08
            messages=msg,
            tools=tools,
...
def eval_rag(eval_filename, output_filename):
    with open(eval_filename) as f, open(output_filename, "w") as of:
        ...
        for line in f:
            ...
            #############################################################
            res_answer = json.loads(response["answer"])
            is_science = res_answer.get("is_science", False)
            if not is_science:
                answer = "과학지식관련 질문이 아닙니다."
                topk = []
            else:
                answer = res_answer['answer']
                topk = response["topk"]
            ...
            output = {"eval_id": j["eval_id"], "standalone_query": response["standalone_query"], 
                      "topk": topk, "answer": answer, "references": response["references"]}
